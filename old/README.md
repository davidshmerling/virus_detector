...
├── Cache/               Scan-result cache (SQLite repository)
...
├── Performance/         Scan section timings and report
...
| `runtime/cache/cache.db` | Clean-file cache (SQLite) |
...
- Cache: every 100 dirty updates (optimization; safe to lose some on crash → only re-scan cost)
- Checkpoint: `min(unfinished_paths_)`; save when that frontier moves, plus forced flush at end of scan
- `markCompleted` is driven by scan/quarantine outcome, not by SQLite COMMIT

## Requirements coverage

| Assignment requirement | Implementation |
|------------------------|----------------|
| Scan cache | `CacheManager` + SQLite repository |
