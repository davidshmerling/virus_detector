...
bool ProgressTracker::markCompleted(const fs::path& relative_path)
{
    ...
    // Mid-scan empty set: workers finished everything discovered so far,
    // but the single enumerator may still find more files. Keep the last
    // frontier path so a crash at most redoes that one file.
...
{
    if (!unfinished_paths_.empty()) {
        // Lexicographic minimum among files not yet completed.
        checkpoint_.next_unfinished_path = *unfinished_paths_.begin();
    } else if (enumeration_finished_) {
        checkpoint_.next_unfinished_path.clear();
    }
    // else: leave next_unfinished_path unchanged while enumeration continues
}
