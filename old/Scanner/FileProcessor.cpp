    ...
    ScanSummary& summary)
{
    ...
    // Cache is an optimization only; completion does not wait for SQLite.
    (void)cache_manager_.update(
        file_path,
        signatures_last_modified_,
        FileVerdict::Clean);
}

bool FileProcessor::handleMaliciousFile(
    const fs::path& file_path,
    ...
    ScanSummary& summary)
{
    ...
    switch (result.verdict) {
        ...
        case FileVerdict::Error:
            // Failures count as completed so one permission error cannot
            // stall the resume frontier forever.
